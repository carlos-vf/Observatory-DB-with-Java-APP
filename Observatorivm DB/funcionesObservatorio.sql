...
CREATE or alter FUNCTION busquedaPorTipo (@tipo varchar(30)) RETURNS table AS
	RETURN (SELECT o.nombre AS NombreObjeto, o.constelacion AS Constelacion, o.descripcion AS Descripcion, 
			...
			FROM Objeto o INNER JOIN TipoObjeto t ON o.tipo = t.id INNER JOIN Descubrimiento d ON o.id = d.id
...
CREATE or alter FUNCTION consultaEquipamiento () RETURNS table AS
	RETURN (SELECT t.id, q.nombre FROM Equipamiento e 
			INNER JOIN TubosOpticos t ON e.idTubo = t.id 
			INNER JOIN EstadoEquipamiento q ON t.estadoEquipamiento = q.id
		UNION 
			SELECT m.id, q.nombre FROM Equipamiento e
			INNER JOIN Montura m ON e.idMontura = m.id
			INNER JOIN EstadoEquipamiento q ON m.estadoEquipamiento = q.id
		UNION 
			SELECT o.id, q.nombre FROM Equipamiento e
			INNER JOIN Oculares o ON e.idOcular = o.id
			INNER JOIN EstadoEquipamiento q ON o.estadoEquipamiento = q.id)
go
...
CREATE or alter FUNCTION FuncionProyectoEstado (@estado varchar(15)) RETURNS table AS
    RETURN (SELECT * FROM Proyecto where proyecto.estadoProyecto = @estado)
go
