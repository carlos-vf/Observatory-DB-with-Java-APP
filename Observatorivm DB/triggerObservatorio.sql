...
BEGIN
	UPDATE Oculares SET estadoEquipamiento = 'descatalogado' WHERE id IN (SELECT id FROM deleted)
	RETURN
END
go
...
BEGIN
	UPDATE Montura SET estadoEquipamiento = 'descatalogado' WHERE id IN (SELECT id FROM deleted)
	RETURN
END
go
...
BEGIN
	UPDATE TubosOpticos SET estadoEquipamiento = 'descatalogado' WHERE id IN (SELECT id FROM deleted)
	RETURN
END
go
...
BEGIN
    ...
    IF NOT EXISTS(SELECT * FROM inserted i INNER JOIN PersonaEstado pe ON i.persona=pe.idPersona 
					WHERE i.fechaObservacion BETWEEN pe.fechaInicio AND pe.fechaFin AND pe.estadoPersona = 'trabajando')
    BEGIN
        ROLLBACK TRANSACTION
...
BEGIN
    ...
    IF NOT EXISTS(SELECT * FROM inserted i INNER JOIN PersonaEstado pe ON i.persona=pe.idPersona 
					WHERE i.persona IS NOT NULL AND (i.fechaDescubrimiento BETWEEN pe.fechaInicio AND pe.fechaFin AND pe.estadoPersona = 'trabajando'))
    BEGIN
        ROLLBACK TRANSACTION
        RAISERROR('La persona no estaba trabajando cuando registró el descubrimiento', 16, 1)
        RETURN
    END
	IF NOT EXISTS(SELECT * FROM inserted i INNER JOIN Proyecto p ON i.proyecto = p.id
					WHERE i.proyecto IS NOT NULL AND (i.fechaDescubrimiento BETWEEN p.fechaIni AND p.fechaFin AND p.estadoProyecto = 'en proceso'))
	BEGIN
        ROLLBACK TRANSACTION
