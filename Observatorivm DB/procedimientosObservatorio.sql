...
CREATE or alter PROCEDURE registroTipoTubo @nombre varchar(30) AS
	BEGIN TRY
		BEGIN TRANSACTION
			IF EXISTS (SELECT id FROM TipoTuboOptico WHERE nombre = @nombre)
				RAISERROR('Ya existe este tipo de tubo optico', 16, 1)
			INSERT INTO TipoTuboOptico(nombre)
				VALUES(@nombre)
		COMMIT TRANSACTION
	END TRY
...
CREATE or alter PROCEDURE registroTipoMontura @nombre varchar(30) AS
	BEGIN TRY
		BEGIN TRANSACTION
		IF EXISTS (SELECT id FROM TipoMontura WHERE nombre = @nombre)
			RAISERROR('Ya existe este tipo de montura', 16, 1)
			INSERT INTO TipoMontura(nombre)
				VALUES(@nombre)
		COMMIT TRANSACTION
	END TRY
...
CREATE or alter PROCEDURE registroDesglosePresupuesto @idProyecto int, @cantidad decimal(10,2), @descripcion varchar(30), @numeroDesglose char(10) AS
	BEGIN TRY
		BEGIN TRANSACTION
...
CREATE or alter PROCEDURE registroDesgloseGasto @idProyecto int, @cantidad decimal(10,2), @descripcion varchar(30), @numeroDesglose char(10) AS
	BEGIN TRY
		BEGIN TRANSACTION
...
CREATE or alter PROCEDURE crearObjeto @id char(10), @tamanho decimal(4,2), @magnitud decimal(4,2), @ar decimal(7,4), @dec decimal(7,4), @nombre varchar(30), 
							 @descripcion varchar(80), @constelacion char(10), @tipo char(10) AS
BEGIN TRY
	BEGIN TRANSACTION
		...
		INSERT INTO Objeto (id, tamanho, magnitud, ar, dec, nombre, descripcion, constelacion, tipo)
			VALUES (@id, @tamanho, @magnitud, @ar, @dec, @nombre, @descripcion, @constelacion, @tipo)
	COMMIT TRANSACTION
END TRY
